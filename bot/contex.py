...
def save_students_come(message):
    ...
    current_time = datetime.now()
    query = """
        INSERT INTO apis_attendens (user_id, attended_time, omad, created_at,is_active,raft)
        VALUES (
...
def save_students_notcome(message, prichina):
    ...
    cur = conn.cursor()
    query = """
        INSERT INTO apis_attendens (user_id, prichina, missed_time, omad, created_at, is_active)
        VALUES (
            (SELECT id FROM apis_users WHERE telegram_id = '%s'),
            %s,
            %s,
...
def update_out(id):
    ...
    query = """
        ...
        SET timeout_time = NOW(), raft = True
